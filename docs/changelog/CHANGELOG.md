...
- Updated `report()` method to include `correlation` check in the list of available checks.
- Added `correlation_threshold` parameter to `report()` method to allow users to customise the correlation level that triggers a warning (default is 0.95).
- Updated Docstrings across the codebase to work with mkdocstrings for automatic API documentation generation.
